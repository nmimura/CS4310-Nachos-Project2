...
public class UserProcess {
    ...
    private int handleCreate(int vAddr) {
        ...
        fileDescriptorTable[nextEmpty] = file;
        return nextEmpty;
    }
    ...
    private int handleRead(int fileDescr, int bufAddr, int count) {
        ...
        OpenFile file = fileDescriptorTable[fileDescr];
        // unable to access file
        if (file == null) {
            return -1;
        }
        ...
        return writeVirtualMemory(bufAddr, buffer, 0, numBytesRead);
    }

    private int handleWrite(int fileDescr, int bufAddr, int count) {
        // invalid fileDescriptorTable index
        if (fileDescr < 0 || fileDescr > 15) {
            return -1;
        }
        ...
        byte[] buffer = new byte[count];
        int numBytesToWrite = readVirtualMemory(bufAddr, buffer, 0, count);
        // error reading virtual memory
        if (numBytesToWrite != count) {
            return -1;
        }

        int numBytesWritten = file.write(buffer, 0, numBytesToWrite);
    ...
    private int handleClose(int fileDescr) {
        ...
        fileDescriptorTable[fileDescr].close();
        fileDescriptorTable[fileDescr] = null;
        return 0;
    }
